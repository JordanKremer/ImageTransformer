...
std::shared_ptr<BmpHeaderInfo> BmpHeaderFactory::getBmpHeader(std::vector<char*>& hData, int compressionFlag) {
	...
	if (compressionFlag == 0)
		return std::make_shared<BmpHeaderInfo_24Bit>(hData, compressionFlag);
	if (compressionFlag == 3)
		return std::make_shared<BmpHeaderInfo_32Bit>(hData, compressionFlag);
