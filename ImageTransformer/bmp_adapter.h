...
{
public:
	std::unique_ptr<generic_image> adapt_from_raw(std::vector<unsigned char>& data);
	const std::vector<unsigned char> adapt_to_raw(std::unique_ptr<generic_image> data);

private:
	std::unique_ptr<generic_image> load_pixels(std::vector<unsigned char>& rawdata, std::unique_ptr<bmp_header_info> header); //can only pass unique by ref or by func(move(ptr))
